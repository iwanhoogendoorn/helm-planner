...
import { parseAssignment, profileFor, shortOf } from '../../src/core/profiles';
...
describe('project profiles', () => {
  it('knows the built-in vocabularies, and lets a project override them', () => {
    ...
    expect(music.modes).toEqual(['Piano with chords', 'Piano', 'Singing', 'Piano with singing']);
    expect(music.short).toEqual(['P CH', 'P', 'Z', 'PZ']);
    ...
    expect(ours.short).toEqual(['Sing', 'Prod']);
    expect(profileFor(undefined).id).toBe('generic');
    expect(profileFor('nonsense').id).toBe('generic');
    expect(shortOf('Piano with singing')).toBe('PS');       // the little words are dropped
  });
  ...
  it('reads an assignment back off a subtask, and leaves anything else alone', () => {
    ...
    expect(parseAssignment('Iwan · Piano with chords', music)).toEqual({ person: 'Iwan', mode: 'Piano with chords' });
    expect(parseAssignment('Piano', music)).toEqual({ mode: 'Piano' });          // nobody named: the work itself
...
describe('adding an item to a profiled project', () => {
  it('makes the month, links the song, and writes a subtask for each person and way', async () => {
    ...
    expect(note).toContain('Iwan · Piano with chords');

    const kids = index.task(song.key)!.childKeys.map((k) => index.task(k)!);
    expect(kids.map((k) => k.text)).toEqual(['Zaara · Singing', 'Iwan · Piano with chords']);
    ...
    const busy = await m.addProfileItem('prj-book', {
      ...
      assignments: [{ person: 'Iwan', mode: 'Piano with chords' }, { person: 'Iwan', mode: 'Piano' }, { person: 'Iwan', mode: 'Singing' }, { person: 'Iwan', mode: 'Piano with singing' }],
    });
