...
export default class HelmPlugin extends Plugin {
  ...
  private registerCommands(): void {
    ...
    this.addCommand({ id: 'catch-up-recurring', name: 'Bring back missed recurring tasks', callback: () => void this.run('Catch up', async () => { const n = await this.mutations.catchUpRecurring(180); new Notice(n === 0 ? 'Every repeating task already has its next turn.' : `Brought back ${n} repeating task${n === 1 ? '' : 's'}.`); }) });
